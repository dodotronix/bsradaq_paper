...
ax.tick_params(axis='both', direction='in', labelcolor='k',
               labelsize='x-large', width=0.6, which='both',
               top=True, right=True)
...
plt.xlabel('Output filter index [-]', fontsize="x-large")
plt.xlim([0, 63])
...
ax2.set_ylabel('Relative Error [\%]', fontsize="x-large")
ax2.set_yticks(np.linspace(-8, 6, 8))
ax2.set_ylim([-6.71, 4.51])

legend = fig.legend(['Operational AGM', 'New AGM', 'Relative Error'], 
         loc='upper left', shadow=False, frameon=True, borderpad=0.5,
         framealpha=1, fontsize="x-large", facecolor='white', fancybox=False, edgecolor=(0, 0, 0),
